...
from info import ADMINS, LOG_CHANNEL, REDEEM_BASE_URL, REDEEM_ACCESS_KEY as ACCESS_KEY
...
async def validate_code(client, message):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(f"{REDEEM_BASE_URL}/?access_key={ACCESS_KEY}&action=validate&code={full_code}") as resp:
            if resp.status == 404:
