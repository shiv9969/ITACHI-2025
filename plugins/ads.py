...
async def set_ads(client, message):
    try:
        ...
        if '#' not in command_args or len(command_args.split('#')) < 3:
            await message.reply_text(f"Usage: /set_ads <ads_name>#<d:duration/i:impression_count>#<url>")
            return
        ...
        # Set the advertisement and stream link in the database
        await mdb.update_advirtisment(reply.text, f"{ads_name}", expiry_date, impression_count)
        await db.set_stream_link(url)
...
async def del_ads(client, message):
    try:
        ...
        if current_link:
            ...
            if is_deleted:
                await message.reply(f"Successfully deleted advertisement and stream link: {current_link}!")
            else:
            ...
            await message.reply("Advertisement reset. Stream link not found!")
    except Exception as e:
